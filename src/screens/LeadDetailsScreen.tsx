    ...
    >
      <Animated.View style={[styles.headerCard, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          ...
          <View style={styles.headerInfo}>
            ...
            <View style={[styles.statusBadge, { backgroundColor: lead.colorType === 'primary' ? colors.textPrimary : colors.limeLight }]}>
              <Text style={[styles.statusText, { color: lead.colorType === 'primary' ? colors.background : colors.textPrimary }]}>
                {t(getStatusTranslationKey(lead.status), lead.status)}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.valueRow}>
          <View>
            <Text style={styles.valueLabel}>{t('leadDetails.dealValue', 'Deal Value')}</Text>
            <Text style={styles.valueText}>{currency}{lead.value}K</Text>
          </View>
          <View>
            <Text style={styles.valueLabel}>{t('leadDetails.probability', 'Probability')}</Text>
      ...
      <Animated.View style={[styles.actionsRow, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        <TouchableOpacity activeOpacity={0.7} style={styles.actionBtn} onPress={handleCall}>
          ...
          <Text style={styles.actionBtnText}>{t('leadDetails.call', 'Call')}</Text>
        </TouchableOpacity>
        <TouchableOpacity activeOpacity={0.7} style={styles.actionBtn} onPress={handleEmail}>
          <MailIcon size={20} color={colors.textPrimary} />
          <Text style={styles.actionBtnText}>{t('leadDetails.email', 'Email')}</Text>
        </TouchableOpacity>
        <TouchableOpacity activeOpacity={0.7} style={styles.actionBtn} onPress={handleSchedule}>
          ...
          <Text style={styles.actionBtnText}>{t('leadDetails.schedule', 'Schedule')}</Text>
        </TouchableOpacity>
      </Animated.View>

      <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        <Text style={styles.sectionTitle}>{t('leadDetails.contactDetails', 'Contact Details')}</Text>
        <View style={styles.detailRow}>
          <MailIcon size={16} color={colors.textSecondary} />
          <Text style={styles.detailText}>{lead.email || t('leadDetails.noEmail', 'No email provided')}</Text>
        </View>
        <View style={styles.detailRow}>
          <PhoneIcon size={16} color={colors.textSecondary} />
          <Text style={styles.detailText}>{lead.phone || t('leadDetails.noPhone', 'No phone provided')}</Text>
        </View>
      </Animated.View>

      <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        <Text style={styles.sectionTitle}>{t('leadDetails.recentActivity', 'Recent Activity')}</Text>
        <View style={styles.activityRow}>
          <View style={styles.activityDot} />
          <View style={styles.activityContent}>
            <Text style={styles.activityText}>{t(`mock.${lead.recentActivity}`, lead.recentActivity || '')}</Text>
            <Text style={styles.activityTime}>{t(`mock.${lead.days}`, lead.days)} {t('leadDetails.ago', 'ago')}</Text>
          </View>
        </View>
      </Animated.View>

      <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        <Text style={styles.sectionTitle}>{t('leadDetails.notes', 'Notes')}</Text>
        <View style={styles.noteCard}>
          <Text style={styles.noteText}>{t(`mock.${lead.notes}`, lead.notes || '') || t('leadDetails.noNotes', 'No notes available.')}</Text>
        </View>
      </Animated.View>
    </Animated.ScrollView>
  );
